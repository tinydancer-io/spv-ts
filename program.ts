...
export function getCopyProgram(rpcUrl: string,privateKey: Uint8Array): Program<Copy> {
...
export async function getCopyAccount(program: Program<Copy>,copy_account_buffer: Buffer): Promise<any> {
  return (await program.account.copyAccount.coder.accounts.decode("copyAccount",copy_account_buffer));
}
